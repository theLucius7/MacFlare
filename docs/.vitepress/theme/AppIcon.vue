...
const icon = computed(() => findAppIcon(props.name, catalog, props.now));
...
watch([() => props.name, () => icon.value?.imageUrl], () => { failed.value = false; });
function imageError(event) {
  // A removed image may report an error after the application has changed.
  if (event.currentTarget?.getAttribute('src') === icon.value?.imageUrl) failed.value = true;
}
</script>

<template>
  <a v-if="icon && !failed" class="app-icon" :style="dimensions" :href="icon.sourceUrl"
    :title="title" :aria-label="`${title} · 图标来源`" target="_blank" rel="noopener noreferrer">
    <img :key="icon.imageUrl" :src="icon.imageUrl" alt="" :width="dimension" :height="dimension"
