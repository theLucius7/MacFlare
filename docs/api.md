...
事件 `seq` 为正安全整数、严格递增；`at` 在窗口内且非降序，同一时间的多个事件按 `seq` 排列。`changes` 非空，仅可含 `active_app`、`running_apps`、`battery`、`system`、`music`，使用 v1 对应字段类型，整个字段替换；对象内部不做深合并。自带 buffered Agent 会保留每次已观测的应用／Music 语义变化，但硬件另按 30 秒采样与阈值合并，并不保留所有微小负载变化；这不改变服务端允许的字段类型或新增拒绝规则。[采集与落盘策略](buffering.md#变化记录、落盘与展示)
...
此响应示例的 `window` 为便于阅读而省略内容，真实返回完整 v2 包（包含上一节所有字段）。`server_time` 用于客户端校准播放时钟；窗口内容不会套用首页的 2 秒视觉合并。`online` 表示窗口仍有效，不表示当前目标播放时刻已有覆盖。首次暖机、缺口和覆盖耗尽由客户端结合窗口判断。[推荐播放行为](buffering.md#缓存、故障与恢复)
...
`online` 只说明服务器最近收到数据；不表示设备此刻可连、用户在场或所有指标均授权。不同边缘读取有传播延迟，详见 [一致性边界](architecture.md#时间、离线与一致性)。
