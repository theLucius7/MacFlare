...
3. 本地先过滤敏感应用，只发送应用名称，不发送窗口标题、路径、命令行、浏览器网页 URL、设备序列号或用户账户名。Music 播放且元数据齐全时，本机可查询 Apple 目录，并附带匹配后的公开封面与歌曲 URL。
...
应用分类接口使用随部署发布的原生图标清单，不向第三方搜索实时应用名。音乐封面由 Mac 原生查询并缓存当前一首，URL 随原有快照推送；Worker 只验证和返回，不请求 Apple 或创建服务端匹配缓存。首页优先使用上报 URL，缺少有效 URL 时保留浏览器兼容查询。本机封面缓存不延长快照的在线期限。[API 契约](api.md) · [封面数据流](privacy.md#音乐-api-与封面上报)
