...
2. 采集只读取用户启用的字段，先隐私过滤再入队；应用／Music 的已观测语义变化立即进入有序内存窗口。硬件 30 秒采样，电量按整数百分比／供电变化、负载按阈值及 120 秒后的成功采样合并，微小负载波动不逐次记录。Music 可使用本机 Apple 搜索补充匹配的封面和歌曲 URL。
3. 最近最多 900 秒保留为一份基线、字段替换事件和明确的采集缺口。普通私有检查点最多每 10 秒合并一次，无变化覆盖心跳 30 秒；启动、睡眠、隐私变化和上传边界立即保存。意外崩溃可能损失最近约 10 秒未保存的普通变化。隐私设置变更清空队列并建立新会话。
...
6. 首页每 60 秒 GET `/api/timeline`，在页面内存中按事件时间播放，正常目标为延时 420 秒。渲染层通常最多每 2 秒切换一次应用／Music／运行列表，短时间连续变化只显示最后候选；原始时间线和 API 不做该视觉合并。缺少覆盖、休眠或采集间断时明确缓冲或缺口，不伪造连续状态。
