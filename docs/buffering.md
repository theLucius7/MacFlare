...
本页集中说明 `buffered` 的固定策略、时序和恢复边界。安装命令见 [快速开始](getting-started.md)，本机开关见 [配置](configuration.md)，请求字段见 [API 参考](api.md)。
...
| 近期封面匹配缓存 | 最多 64 首；成功 900 秒、失败 300 秒 | 本机内存缓存；派生 URL 随窗口保存，不新增单独上传 |
...
| 包大小／条目上限 | 512 KiB／2048 个事件／128 个缺口 | 内存、磁盘、上传和解析有界；溢出计数并推进窗口起点 |
...
成功上传后等待 300 秒；失败以 15 秒为起点指数退避，乘以 0.8–1 的随机系数，最长等待 300 秒。每次只发送当前最新完整窗口，不逐包追发旧请求。恢复有效检查点时保留原上传计划，并限制到当前时间至 300 秒之后；重新获得新鲜观察后再上传，避免每次重启都重新等待整轮。
...
`STATUS_TTL_SECONDS` 继续控制旧快照模式，不改变 v2 窗口策略。迁移必须先发布新版 Worker，再更新当前用户的 Agent；新 Worker 同时接受旧 `/api/update`，旧 Worker 没有 `/api/batch`。具体升级命令与回退步骤集中在 [部署与维护](deployment.md#升级滑动窗口模式)。
