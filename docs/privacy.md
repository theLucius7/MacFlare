...
应用图标在部署前由维护者同步。macOSicons 搜索 API 接收维护者配置的少量固定应用名及其鉴权请求，不接收逐个访客的实时运行列表；API Key 不进入 Git、生成记录或浏览器。配置本身是公开的应用映射，不应包含私有应用信息。

访客浏览器使用已发布的映射，从图标 CDN 加载原始 `lowResPngUrl`。CDN 会收到对应图片请求和访客网络信息，这些请求可能反映页面正在展示的应用；不携带电池、负载、曲目、完整运行列表或令牌。未知应用和隐私屏蔽后的 `System` 使用占位。

同步记录最多有效 30 天，到期不再展示；它们不进入 KV，也不增加 `/api/now` 字段。图标作者与 macOSicons 来源会保留在页面的标题和链接中。配置、更新和授权边界见 [应用图标](app-icons.md)。
