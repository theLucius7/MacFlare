...
buffered 是短期活动缓冲：本机以权限 `600` 缓存最近最多 15 分钟的基线、变化和缺口；Worker 只保留最新接收的完整窗口，按 `window_end + 600 秒` 绝对过期。一个窗口最早的事件因此最多跨越约 25 分钟的云端有效范围。不能把它描述成“无历史”：访问者可以读取该窗口内的应用切换和曲目顺序。原始按键、鼠标点击、文档、网页历史与屏幕内容不在采集范围。
