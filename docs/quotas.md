...
buffered 只把达到记录条件的字段变化加入事件队列：应用／Music 已观测变化直接记录，硬件用 30 秒采样、整数电量与负载阈值减少细微波动事件。普通检查点最多每 10 秒合并落盘，无变化时每 30 秒保存覆盖心跳；这减少本机写入，与 KV 上传次数分开。不过窗口仍需向前移动，让访问者知道已经观测到哪一刻。完全停止上传会让云端过期，也无法区分静止状态与设备失联。因此变化去重减少包体，固定上传间隔减少 KV 写入；二者各有用途。
...
首页在 buffered 下每 60 秒读取一次窗口，兼容 v1 时恢复 120 秒间隔，页面隐藏时暂停；其他文档页面不自动加载状态。一个持续打开的主页约读 1,440 次/日，多个访问者会累加，重新打开页面另计。事件播放与画面变化不发起 KV 请求。首页通常最多每 2 秒合并一次普通切换只减少视觉更新，不降低每次已发生 GET 的 KV 计数。静态文档、样式和本地搜索不访问 KV。
...
KV 最终一致；延时缓冲能吸收部分抖动，不能保证传播延迟上限或跨地区连续播放。[一致性边界](architecture.md#时间、离线与一致性)
