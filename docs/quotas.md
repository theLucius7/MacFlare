...
每个页面使用最多 50 条的内存缓存：成功结果 1 小时，普通查询失败或无可信匹配 5 分钟；取消请求不缓存，不写入持久存储。缓存到期只影响下次查询，不会自动重试相同曲目。不同访客、重新打开页面仍可能产生独立请求；Apple 的服务限制与网络可用性不属于 Cloudflare 配额。封面缓存时长不延长设备状态的有效期。[数据流与隐私](privacy.md#首页歌曲封面)
